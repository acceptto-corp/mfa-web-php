...
class crud
{
    ...
    public function __isset($name)
    {
		...
		public function insertAccessToken($email, $access_token)
		{				
           $sql = "UPDATE users SET m2m_access_token='".$access_token."', m2m_authenticated= '1' WHERE name='".$email."';";
           $this->rawQuery($sql);
		}
		...
		{				
           $sql = "SELECT m2m_access_token FROM users WHERE name='".$email."';";
           $records = $this->rawSelect($sql);
		   
		   $rows = $records->fetchAll(PDO::FETCH_ASSOC);
		   return $rows[0]['m2m_access_token'];
		}
